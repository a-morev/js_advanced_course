...
class ProductList {
  ...
  renderTotal() {
    document.getElementById("cart-total").innerHTML = `В корзине ${
      this.getCartItems().length
    } товаров на ${this.getGoodsPrice()} \u20bd`;

    document.getElementById("cart-items").innerHTML = this.getCartItems()
      .map(
        (item) =>
          `<ul>${item.title} <button data-id="${item.id}" class="remove-btn">x</button></ul>`
      )
      .join("");

    Array.from(document.querySelectorAll(".remove-btn")).forEach((el) => {
